...
public class GentlyDownTheStream {
    ...
    public List<String> reverseSortedVeggiesInUpperCase() {
        return veggies.stream().sorted(Comparator.reverseOrder()).map(x -> x.toUpperCase()).collect(Collectors.toList());
    }
    ...
    public List<Integer> topTenUniqueOdd() {
        return integerValues.stream().filter(x -> x % 2 == 1).sorted(Comparator.reverseOrder()).distinct().limit(10).collect(Collectors.toList());
    }
